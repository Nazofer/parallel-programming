...
func printRootUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  lab2 run <tags|stats|matmul|transactions> [flags]")
	fmt.Fprintln(w, "  lab2 bench <tags|stats|matmul|transactions> [flags]")
	fmt.Fprintln(w, "  lab2 generate <html-dataset|transactions-dataset> [flags]")
}
...
func generateCommand(args []string, stdout, stderr io.Writer) error {
	...
	switch args[0] {
	case "html-dataset":
		cfg, err := parseHTMLDatasetFlags(args[1:], stderr)
		if err != nil {
			return err
		}
		stats, err := generateHTMLDataset(cfg)
		if err != nil {
			return err
		}
		fmt.Fprintln(stdout, "HTML dataset generated")
		fmt.Fprintf(stdout, "Root: %s\n", cfg.Root)
		fmt.Fprintf(stdout, "Directories: %d\n", stats.Directories)
		fmt.Fprintf(stdout, "Files: %d\n", stats.Files)
		fmt.Fprintf(stdout, "Tags: %d\n", stats.Tags)
		return nil
	case "transactions-dataset":
		cfg, err := parseTransactionsDatasetFlags(args[1:], stderr)
		if err != nil {
			return err
		}
		stats, err := generateTransactionsDataset(cfg)
		if err != nil {
			return err
		}
		fmt.Fprintln(stdout, "Transactions dataset generated")
		fmt.Fprintf(stdout, "Root: %s\n", cfg.Root)
		fmt.Fprintf(stdout, "Files: %d\n", stats.Files)
...
func benchmarkTask(task string, variants []executionVariant, workersList []int, repeat int, taskArgs []string, stderr io.Writer) ([]benchRow, map[string]string, error) {
	...
	for i := 0; i < repeat; i++ {
		result, err := executeTask(task, variantSeq, 1, taskArgs, stderr)
		if err != nil {
			return nil, nil, err
		}
		seqResult = result
...
func executeTask(task string, variant executionVariant, workers int, taskArgs []string, stderr io.Writer) (TaskResult, error) {
	...
	switch task {
	case "tags":
		cfg, err := parseTagsFlags(taskArgs, stderr)
		if err != nil {
			return TaskResult{}, err
		}
		return runTagsTask(cfg, variant, workers)
	case "stats":
		cfg, err := parseStatsFlags(taskArgs, stderr)
		if err != nil {
			return TaskResult{}, err
		}
		return runStatsTask(cfg, variant, workers)
	case "matmul":
		cfg, err := parseMatmulFlags(taskArgs, stderr)
		if err != nil {
			return TaskResult{}, err
		}
		return runMatmulTask(cfg, variant, workers)
	case "transactions":
		cfg, err := parseTransactionsFlags(taskArgs, stderr)
		if err != nil {
			return TaskResult{}, err
		}
		return runTransactionsTask(cfg, variant, workers)
	default:
